...
1. `element.currentStyle.width/height;` 弥补了`element.style.width`的不足，但只IE特有
2. `window.getComputedStyle(element).width/height;` 通用型，跟`currentStyle`作用一样，但可以支持Chrome、FireFox等
...
#### BFC
**Box** 是 CSS布局中的基本单位。
简单来说，若干个Box(盒模型)组成了一个页面。在HTML中，每个元素都有自己默认的Box类型。但是我们也可手动更改它们的Box类型。不同类型的Box，有不同的渲染规则。比如它们的子元素如何定位，以及子元素的之间的相互作用关系。
...
<body>
    ...
    <div class="div2">
        <p>
            This is div2<br>
            <strong style="color:red;">根据边框，我们可以看到“div2” 跟“div1”有重叠部分， div2占据了原先div1（为设置float属性前）的空间。但是我们发现div2中的文字并没有占据div1的空间，而是紧挨着div1</strong>
        </p>
    </div>
</body>
</html>
