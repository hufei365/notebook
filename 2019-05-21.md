...
可以通过添加`crossorigin`属性让处于同域下的模块脚本请求时，携带credentials信息（这看起来有些奇怪，我已经提出了[问题](https://github.com/whatwg/html/issues/2557)）。如果你想在请求其它域下的模块脚本时，可以使用`crossorigin="use-credentials"`。需要留意的是，其它域返回信息中需要`Access-Control-Allow-Credentials: true` 响应头。
